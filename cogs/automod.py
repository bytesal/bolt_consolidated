...
class AutoModCog(commands.Cog):
    ...
    async def get_settings(
        ...
        settings = await db_cog.automod.find_one({
        ...
        if not settings:
            ...
            await db_cog.automod.insert_one(
                settings
            )
    ...
    async def on_message(
            ...
            if re.search(
                ...
                if (
                    ...
                    warning = await message.channel.send(
                        ...
                        f"{message.author.mention} "
                        f"❌ Advertising is only allowed "
                        f"in designated channels."
        ...
        if settings.get("anti_spam"):
            ...
            self.spam_cache[user_id].append(
                now
            )

            spam_seconds = settings.get(
                "spam_seconds",
                6
            )
            ...
            self.spam_cache[user_id] = [
                ...
                for timestamp
                in self.spam_cache[user_id]

                if now - timestamp <= spam_seconds
            ]

            spam_messages = settings.get(
                "spam_messages",
                5
            )
    ...
    async def automod_mentions(
        ...
        await ctx.send(
            f"✅ Anti-mentions set to `{enabled}`"
        )
    ...
    async def allow_ads(
        ...
        confirmation = await ctx.send(
            "✅ Ads are now allowed in this channel."
        )
